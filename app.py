...
This is the back-end code to run the server on which a user can upload and
select the data of which they wish to extract.
...
def selects():
    ...
    if selection[0] == 1 or selection[2] == 1:
        ...
        tesseract_exe = os.path.join(tess_exec_0, "tesseract.exe")
        ...
        ir = ImageReader(OS.Windows)  # Required for reading in tesseract.exe
    ...
    """
    Above is Reading the Data via OCR Method (for if data in PDF is picture
    based)

    Below is Reading the Data by Read Method - Reading the text or the
    annotations (for if data in PDF is written based)
    """
    ...
    if selection[1] == 1 or selection[2] == 1:
        ...
        for page_id, page in enumerate(reader.pages):
            ...
            tot_list = [text_drawing_title, text_drawing_number,
                        text_drawing_stat, text_drawing_date,
                        text_drawing_revision, text_drawing_pap,
                        text_drawing_scal, text_drawing_pagnum]
...
# This combines both data from OCR and Reading into once CSV file to compare
    def Convert(lst):
...
# localhost == 127.0.0.1 --> this is the local host, thus points to your own computer
